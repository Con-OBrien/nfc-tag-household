...
export class MembershipController {
  ...
  async inviteUser(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const requesterId = (req as any).userId; // Set by auth middleware
      ...
      if (!isOwner) {
        throw new ForbiddenError('Only household owners can invite members');
      }

      // Check if user already exists in household
      const existingUser = await this.userRepository.findByEmail(email, householdId);
      if (existingUser) {
        throw new ValidationError(`User with email "${email}" is already a member of this household`);
      }
  ...
  async updateUserRole(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      if (!householdId || !memberId) {
        throw new ValidationError('householdId and memberId are required');
      }
      ...
      // Verify requester is in household
      const isRequesterInHousehold = await this.householdRepository.isUserInHousehold(
        requesterId,
        householdId
      );
      ...
      if (!isRequesterOwner) {
        throw new ForbiddenError('Only household owners can change member roles');
      }
      ...
      if (member.role === 'owner' && role === 'member') {
        ...
        if (owners.length === 1) {
          throw new ValidationError('Cannot remove the last owner from a household');
        }
      }
  ...
  async removeMember(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const isRequesterOwner = await this.householdRepository.isUserOwner(requesterId, householdId);
      if (!isRequesterOwner) {
        throw new ForbiddenError('Only household owners can remove members');
      }
   ...
   */
  async getHouseholdMembers(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const householdId = Array.isArray(req.params.householdId) 
        ? req.params.householdId[0] 
        : req.params.householdId;
   ...
   */
  async getMemberDetails(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const householdId = Array.isArray(req.params.householdId) 
        ? req.params.householdId[0] 
        : req.params.householdId;
