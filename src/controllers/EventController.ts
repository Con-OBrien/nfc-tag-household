...
import { TaskValidatorService } from '../services/TaskValidatorService';
import { NotificationService } from '../services/NotificationService';
import { NFCReaderService } from '../services/NFCReaderService';
import {
  ValidationError,
  ForbiddenError,
  NotFoundError,
...
export class EventController {
  ...
  private nfcReader: NFCReaderService;
  ...
  constructor() {
    ...
    this.eventProcessor = new EventProcessorService(this.eventRepository);
    this.taskValidator = new TaskValidatorService(taskRepository, this.userRepository);
    ...
    this.nfcReader = new NFCReaderService();
  }
   ...
   */
  async tagScanned(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      if (!tagData || typeof tagData !== 'string') {
        throw new ValidationError('tagData is required and must be a string');
      }
      ...
      if (!householdId || typeof householdId !== 'string') {
        throw new ValidationError('householdId is required and must be a string');
      }

      if (typeof timestamp !== 'number' || timestamp <= 0) {
        throw new ValidationError('timestamp is required and must be a positive number');
      }

      // Validate timestamp is not too old (more than 5 minutes)
      const fiveMinutesMs = 5 * 60 * 1000;
      if (Date.now() - timestamp > fiveMinutesMs) {
        throw new ValidationError('Tag scan is too old (must be within 5 minutes)');
      }

      // Step 2: Parse NFC tag data
      let nfcData: NFCTagData;
      try {
        ...
        if (!nfcData.tagId || !nfcData.taskId) {
          throw new ValidationError('Tag data must contain tagId and taskId');
        }
      } catch (e) {
        throw new ValidationError(`Failed to parse tag data: ${e instanceof Error ? e.message : String(e)}`);
      }
      ...
      if (tagSignature) {
        ...
        if (!isValidSignature) {
          throw new ForbiddenError('Invalid tag signature');
        }
      }
      ...
      if (!userInHousehold) {
        throw new ForbiddenError('User does not belong to this household');
      }

      // Step 5: Validate task and user permissions
      const validationResult = await this.taskValidator.validateTaskCanExecute(
        userId,
        nfcData.taskId,
        householdId
      );
      ...
      if (!event) {
        ...
        throw new ValidationError('Duplicate scan detected (duplicate within 30 seconds)');
      }

      // Step 7: Validate event integrity
      if (!this.eventProcessor.validateEventIntegrity(event)) {
        throw new ValidationError('Event integrity validation failed');
      }
