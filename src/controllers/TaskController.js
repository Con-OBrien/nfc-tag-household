...
class TaskController {
    ...
    async createTask(req, res, next) {
        try {
            ...
            if (!name || !description || !category) {
                throw new types_1.ValidationError('name, description, and category are required');
            }
            ...
            if (typeof description !== 'string' || description.length < 1 || description.length > 500) {
                throw new types_1.ValidationError('Task description must be between 1 and 500 characters');
            }
            // Validate category is non-empty string
            if (typeof category !== 'string' || category.length < 1) {
                throw new types_1.ValidationError('Task category must be a non-empty string');
            }
    ...
    async updateTask(req, res, next) {
        try {
            ...
            if (!householdId || !taskId) {
                throw new types_1.ValidationError('householdId and taskId are required');
            }
            ...
            if (existingTask.createdBy !== userId) {
                throw new types_1.ForbiddenError('Only the task owner can update this task');
            }
            ...
            const updatedTask = await this.taskRepository.updateTask(taskId, householdId, updates);
    ...
    async deactivateTask(req, res, next) {
        try {
            ...
            if (existingTask.createdBy !== userId) {
                throw new types_1.ForbiddenError('Only the task owner can deactivate this task');
            }
            // Deactivate the task
        ...
        try {
            ...
            if (activeStr !== undefined) {
                if (activeStr === 'true') {
                    ...
                    throw new types_1.ValidationError('active parameter must be true or false');
                }
            }
            // Build filter object
            const filters = {};
