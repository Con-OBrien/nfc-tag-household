...
import { NFCTagData, TaskEvent } from '../types';
...
export class EventProcessorService {
  ...
  private deduplicationWindow: Map<string, number> = new Map(); // key: userId:taskId, value: timestamp
   ...
   * Requirements: 2.2
   */
  public enrichEventWithUserContext(
    event: TaskEvent,
    enrichmentData: {
      userId: string;
      householdId: string;
   ...
   * Requirements: 2.3
   */
  public validateEventIntegrity(event: TaskEvent): boolean {
   ...
   * Requirements: 1.4
   */
  public async getRecentEventsForTask(
    taskId: string,
    userId: string,
    householdId: string
  ): Promise<TaskEvent[]> {
   ...
   */
  public cleanupOldDeduplicationEntries(): void {
   ...
   * Requirements: 1.4
   */
  private isDuplicateScan(userId: string, taskId: string, timestamp: number): boolean {
   ...
   * @param timestamp - The scan timestamp
   */
  private recordScan(userId: string, taskId: string, timestamp: number): void {
    const key = `${userId}:${taskId}`;
