...
exports.DeliveryRetryService = void 0;
/**
...
class DeliveryRetryService {
    constructor(eventRepository) {
        this.maxAttempts = 3;
        this.backoffDelays = [1000, 2000, 4000]; // 1s, 2s, 4s in milliseconds
     ...
     * Requirements: 5.4, 5.5, 5.6, 14.3, 14.4
     */
    async executeWithRetry(deliveryFn, pushToken, userId, eventId, taskId, householdId) {
        const attempts = [];
        ...
        for (let attemptNumber = 1; attemptNumber <= this.maxAttempts; attemptNumber++) {
            try {
                ...
                if (success) {
                    ...
                    console.log(`Delivery succeeded for user ${userId} on attempt ${attemptNumber}`);
                    break;
                }
                else {
                ...
                console.error(`Attempt ${attemptNumber} failed for user ${userId}: [${classification}] ${errorMessage}`);
                // Don't retry if permanent failure
                if (classification === 'permanent_failure' || classification === 'token_expired') {
                    console.warn(`Permanent failure detected (${classification}). Stopping retry attempts.`);
                    break;
                }
                // Wait before retrying (except on last attempt)
                if (attemptNumber < this.maxAttempts) {
                    const delayMs = this.backoffDelays[attemptNumber - 1];
                    console.log(`Retrying in ${delayMs}ms (attempt ${attemptNumber + 1}/${this.maxAttempts})`);
        ...
        try {
            await this.updateAuditLogWithDeliveryStatus(eventId, userId, taskId, householdId, deliveryStatus);
        }
        catch (auditError) {
        ...
        if (finalStatus === 'delivered') {
            console.log(`✓ Delivery succeeded for user ${userId} on attempt ${attempts.length}/${this.maxAttempts}`);
        }
        else {
            console.log(`✗ Delivery failed for user ${userId} after ${attempts.length} attempts`);
        }
     ...
     * Requirements: 14.4
     */
    async updateAuditLogWithDeliveryStatus(eventId, userId, taskId, householdId, deliveryStatus) {
        try {
            ...
            const deliveryEvent = {
                ...
                taskId,
                householdId,
                action: 'acknowledge', // Use 'acknowledge' action to track delivery confirmation
                timestamp: Date.now(),
                metadata: {
            ...
            console.log(`Audit log updated: Delivery status for event ${eventId} logged successfully`);
        }
        catch (error) {
            console.error(`Failed to log delivery status for event ${eventId}:`, error);
...
exports.DeliveryRetryService = DeliveryRetryService;
