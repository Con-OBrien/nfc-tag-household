...
const DeliveryRetryService_1 = require("./DeliveryRetryService");
const PushServiceFactory_1 = require("./PushServiceFactory");
/**
...
class NotificationService {
    constructor(userRepository, householdRepository, taskRepository, eventRepository) {
        this.userRepository = userRepository;
        this.householdRepository = householdRepository;
        this.taskRepository = taskRepository;
        this.eventRepository = eventRepository;
        this.retryService = new DeliveryRetryService_1.DeliveryRetryService(eventRepository);
    }
    /**
     ...
     */
    async sendNotification(payload, recipients, eventId, taskId, householdId) {
        ...
        for (const recipient of recipients) {
            ...
            try {
                ...
                const deliveryFn = async () => this.deliverNotification(payload, recipient.pushToken, this.detectPlatform(recipient.pushToken));
                // Execute with retry logic and failure classification
                // Use provided event context or defaults for audit logging
                const deliveryStatus = await this.retryService.executeWithRetry(deliveryFn, recipient.pushToken, recipient.userId, eventId || 'unknown-event', taskId || 'unknown-task', householdId || 'unknown-household');
     ...
     */
    async sendToHouseholdMembers(taskId, event, excludeUser) {
        try {
            ...
            const householdMembers = await this.userRepository.findUsersInHousehold(event.householdId);
            ...
            const eligibleRecipients = householdMembers.filter((member) => member.userId !== excludeUser && member.pushToken);
            if (eligibleRecipients.length === 0) {
                console.log(`No eligible recipients for notification in household ${event.householdId}`);
                return;
            }
            ...
            if (filteredRecipients.length === 0) {
                console.log(`All recipients filtered out by preferences for task ${taskId}`);
                return;
            }
            ...
            const result = await this.sendNotification(payload, filteredRecipients, event.eventId, task.taskId, event.householdId);
            console.log(`Notification sent: ${result.delivered.length} delivered, ${result.failed.length} failed`);
        }
        catch (error) {
     ...
     */
    filterByPreferences(recipients, task) {
        ...
        for (const recipient of recipients) {
            ...
            if (!recipient.preferences) {
                console.warn(`No preferences found for user ${recipient.userId}, skipping notification`);
                continue;
            }
            // Check 1: Notifications enabled (fail fast on first condition)
            if (!this.isNotificationEnabled(recipient)) {
                console.log(`[PREFERENCE] Notifications disabled for user ${recipient.userId}`);
            ...
            if (!this.isTaskNotMuted(recipient, task.taskId)) {
                console.log(`[PREFERENCE] Task ${task.taskId} is muted for user ${recipient.userId}`);
                continue; // Early exit to next recipient
            }
            // Check 3: Not in quiet hours (fail fast on third condition)
            if (!this.isOutsideQuietHours(recipient)) {
                console.log(`[PREFERENCE] User ${recipient.userId} is in quiet hours (no notification sent)`);
                continue; // Early exit to next recipient
            }
            // All preference checks passed: add to filtered list
            console.log(`[PREFERENCE] User ${recipient.userId} passes all preference checks - notification eligible`);
     ...
     * Requirements: 9.2
     */
    isNotificationEnabled(recipient) {
     ...
     */
    isTaskNotMuted(recipient, taskId) {
        // Handle missing or null preferences
        if (!recipient || !recipient.preferences) {
    ...
    isOutsideQuietHours(recipient) {
        ...
        if (!this.isValidQuietHours(recipient.preferences.quietHours)) {
            console.warn(`Invalid quiet hours configuration for user ${recipient.userId}: ${JSON.stringify(recipient.preferences.quietHours)}`);
            return true; // Default to allowing notification if hours are invalid
        }
        // Check if currently outside quiet hours
        const isInQuietHours = this.isCurrentlyInQuietHours(recipient.preferences.quietHours);
     ...
     */
    isCurrentlyInQuietHours(quietHours) {
        const now = new Date();
     ...
     */
    formatNotificationPayload(task, event) {
     ...
     * Requirements: 5.3
     */
    detectPlatform(pushToken) {
        return PushServiceFactory_1.PushServiceFactory.detectPlatform(pushToken);
    }
    /**
     ...
     */
    async deliverNotification(payload, pushToken, platform) {
        try {
            // Use PushServiceFactory to route to appropriate platform service
            return await PushServiceFactory_1.PushServiceFactory.sendPushNotification(pushToken, payload);
        }
        catch (error) {
