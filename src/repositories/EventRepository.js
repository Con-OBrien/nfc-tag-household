...
const TaskEvent_1 = require("../entities/TaskEvent");
...
class EventRepository {
    constructor() {
        this.repository = database_1.AppDataSource.getRepository(TaskEvent_1.TaskEventEntity);
    }
    /**
    ...
    async logEvent(event) {
        ...
        if (!event.userId || !event.taskId || !event.householdId || !event.action) {
            throw new types_1.ValidationError('userId, taskId, householdId, and action are required fields for event logging');
        }
        if (!['execute', 'acknowledge', 'undo', 'comment'].includes(event.action)) {
            throw new types_1.ValidationError(`Invalid action "${event.action}". Must be one of: execute, acknowledge, undo, comment`);
        }
        if (!event.timestamp) {
            throw new types_1.ValidationError('timestamp is required for event logging');
        }
        ...
        if (Math.abs(currentTime - event.timestamp) > dayInMs) {
            throw new types_1.ValidationError(`Event timestamp must be within 24 hours of current time. Provided: ${event.timestamp}, Current: ${currentTime}`);
        }
     ...
     */
    async getEventHistory(taskId, householdId, limit = 100) {
     ...
     */
    async getEventsByUser(userId, householdId, limit = 100) {
     ...
     */
    async queryEventsByDateRange(startTime, endTime, householdId) {
        if (!householdId) {
            throw new types_1.ValidationError('householdId is required for security isolation');
        }
        if (!startTime || !endTime) {
            throw new types_1.ValidationError('startTime and endTime are required');
        }
        if (startTime > endTime) {
            throw new types_1.ValidationError('startTime must be less than or equal to endTime');
        }
        ...
        if (endTime - startTime > maxWindowMs) {
            throw new types_1.ValidationError('Date range cannot exceed 90 days');
        }
        const eventEntities = await this.repository
            .createQueryBuilder('event')
     ...
     */
    preventModification() {
        throw new Error('Cannot modify or delete task events. Events are immutable and can only be created. ' +
     ...
     */
    async countEventsByAction(householdId) {
        if (!householdId) {
            throw new types_1.ValidationError('householdId is required');
        }
