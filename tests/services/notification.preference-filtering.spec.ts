...
import { Task, HouseholdUser, NotificationPayload } from '../../src/types';
...
describe('NotificationService - Preference Filtering', () => {
  ...
  // Helper functions
  const createHouseholdUser = (overrides?: Partial<HouseholdUser>): HouseholdUser => ({
    userId: 'user-123',
    householdId,
    name: 'Test User',
  ...
  const createNotificationPayload = (): NotificationPayload => ({
  ...
  beforeEach(() => {
    ...
    mockHouseholdRepository = {} as any;
    mockTaskRepository = {
  ...
  describe('Edge Cases - Null/Undefined Handling', () => {
    it('should handle empty recipients array', async () => {
      const recipients: HouseholdUser[] = [];
      const task = createTask();
