...
const DeliveryRetryService_1 = require("../../src/services/DeliveryRetryService");
...
describe('DeliveryRetryService', () => {
    ...
    beforeEach(() => {
        ...
        retryService = new DeliveryRetryService_1.DeliveryRetryService(mockEventRepository);
    });
    describe('executeWithRetry - Success Scenarios', () => {
        it('should return delivered status on first attempt success', async () => {
            ...
            const result = await retryService.executeWithRetry(deliveryFn, testData.pushToken, testData.userId, testData.eventId, testData.taskId, testData.householdId);
    ...
    describe('executeWithRetry - Exponential Backoff', () => {
        ...
        it('should respect exponential backoff timing between retries', async () => {
            ...
            await retryService.executeWithRetry(deliveryFn, testData.pushToken, testData.userId, testData.eventId, testData.taskId, testData.householdId);
    ...
    describe('executeWithRetry - Audit Logging', () => {
        ...
        it('should include all attempts in audit log metadata', async () => {
            ...
            expect(loggedEvent.metadata.attempts[0].classification).toBe('temporary_failure');
        ...
        it('should mask push token in audit log for security', async () => {
            const deliveryFn = async () => true;
            await retryService.executeWithRetry(deliveryFn, 'very_long_test_token_12345678', testData.userId, testData.eventId, testData.taskId, testData.householdId);
            const loggedEvent = mockEventRepository.logEvent.mock
                .calls[0][0];
