import { PushServiceFactory } from '../../src/services/PushServiceFactory';
...
describe('PushServiceFactory', () => {
  ...
  describe('detectPlatform', () => {
    it('should detect APNs tokens (64 hex characters)', () => {
      ...
      for (const token of apnsTokens) {
        expect(PushServiceFactory.detectPlatform(token)).toBe('apns');
      }
    });
    ...
    it('should detect FCM tokens (non-64 hex, valid base64-url format)', () => {
      ...
      for (const token of fcmTokens) {
        expect(PushServiceFactory.detectPlatform(token)).toBe('fcm');
      }
    });
  ...
  describe('initializeServices', () => {
    it('should initialize both FCM and APNs services', () => {
      ...
      PushServiceFactory.initializeServices();

      expect(PushServiceFactory.isFCMReady()).toBe(true);
      expect(PushServiceFactory.isAPNsReady()).toBe(true);
    });
    ...
    it('should handle FCM initialization failure gracefully', () => {
      ...
      expect(PushServiceFactory.isFCMReady()).toBe(false);
      expect(PushServiceFactory.isAPNsReady()).toBe(true);
    });
    ...
    it('should handle APNs initialization failure gracefully', () => {
      ...
      expect(PushServiceFactory.isFCMReady()).toBe(true);
      expect(PushServiceFactory.isAPNsReady()).toBe(false);
    });
    ...
    it('should track initialization errors', () => {
      ...
      const errors = PushServiceFactory.getInitializationErrors();
  ...
  describe('sendPushNotification', () => {
    ...
    it('should route APNs tokens to APNs service', async () => {
      ...
      const result = await PushServiceFactory.sendPushNotification(apnsToken, mockNotificationPayload);
    ...
    it('should route FCM tokens to FCM service', async () => {
      ...
      const result = await PushServiceFactory.sendPushNotification(fcmToken, mockNotificationPayload);
    ...
    it('should throw error if target platform service is not initialized', async () => {
      ...
      await expect(
        PushServiceFactory.sendPushNotification(fcmToken, mockNotificationPayload)
      ).rejects.toThrow('FCM service not initialized');
    });
    ...
    it('should propagate service-specific errors', async () => {
      ...
      await expect(
        PushServiceFactory.sendPushNotification(invalidAPNsToken, mockNotificationPayload)
  ...
  describe('service access methods', () => {
    ...
    it('should provide access to FCM service instance', () => {
      const fcmService = PushServiceFactory.getFCMService();
      expect(fcmService).not.toBeNull();
      expect(fcmService).toBeInstanceOf(FCMService);
    });

    it('should provide access to APNs service instance', () => {
      const apnsService = PushServiceFactory.getAPNsService();
      expect(apnsService).not.toBeNull();
      expect(apnsService).toBeInstanceOf(APNsService);
    });
    ...
    it('should return null for uninitialized services', () => {
      ...
      expect(PushServiceFactory.getFCMService()).toBeNull();
      expect(PushServiceFactory.getAPNsService()).toBeNull();
    });
  });
  ...
  describe('platform detection accuracy', () => {
    it('should not confuse similar length tokens', () => {
      ...
      const result = PushServiceFactory.detectPlatform(hexTokenLike);
    ...
    it('should handle case-insensitive APNs tokens', () => {
      ...
      expect(PushServiceFactory.detectPlatform(lowercaseToken)).toBe('apns');
      expect(PushServiceFactory.detectPlatform(uppercaseToken)).toBe('apns');
      expect(PushServiceFactory.detectPlatform(mixedToken)).toBe('apns');
    });
  });
  ...
  describe('initialization error handling', () => {
    it('should handle partial initialization correctly', () => {
      ...
      expect(PushServiceFactory.isFCMReady()).toBe(true);
      expect(PushServiceFactory.isAPNsReady()).toBe(false);
      expect(PushServiceFactory.getInitializationErrors().size).toBeGreaterThan(0);
    });
  });
  ...
  describe('reset functionality', () => {
    ...
    it('should clear initialization errors on reset', () => {
      ...
      expect(PushServiceFactory.getInitializationErrors().size).toBe(0);
    });
  });
});
