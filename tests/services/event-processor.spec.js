...
describe('EventProcessorService', () => {
    ...
    beforeEach(() => {
        ...
        service = new EventProcessorService_1.EventProcessorService(mockEventRepository);
    });
    describe('processTagEvent', () => {
        ...
        it('should allow same user on different tasks', async () => {
            ...
            expect(result1).not.toBeNull();
            // Scan for task 2
            const result2 = await service.processTagEvent({ ...validNFCData, taskId: 'task_002' }, userId, householdId);
            expect(result2).not.toBeNull();
            expect(mockEventRepository.logEvent).toHaveBeenCalledTimes(2);
        });
        it('should throw error if event persistence fails', async () => {
            ...
            await expect(service.processTagEvent(validNFCData, 'user_001', 'household_001')).rejects.toThrow('Database error');
    ...
    describe('validateEventIntegrity', () => {
        ...
        it('should reject event with invalid action', () => {
            const invalidEvent = { ...validEvent, action: 'invalid' };
            expect(service.validateEventIntegrity(invalidEvent)).toBe(false);
        });
    ...
    describe('cleanupOldDeduplicationEntries', () => {
        it('should remove entries older than 1 minute', async () => {
            ...
            await service.processTagEvent({ ...validNFCData, timestamp: Date.now() + 31 * 1000 }, 'user_001', 'household_001');
            // Cleanup
    ...
    describe('Integration tests', () => {
        ...
        it('should properly isolate deduplication by user and task', async () => {
            ...
            const result1b = await service.processTagEvent({ ...validNFCData, taskId: 'task_002' }, 'user_001', 'household_001');
