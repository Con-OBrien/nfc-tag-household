...
describe('FCMService', () => {
  ...
  describe('constructor and initialization', () => {
    ...
    it('should throw error on invalid project ID format', () => {
      expect(() => {
        new FCMService('invalid PROJECT ID!', '');
  ...
  describe('sendMessage - successful delivery', () => {
    ...
    it('should deliver payload with correct structure', async () => {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith(
        expect.stringContaining('[FCM] Notification delivered')
      );
  ...
  describe('sendMessage - invalid token handling', () => {
    ...
    it('should throw error on empty token', async () => {
      await expect(fcmService.sendMessage('', mockNotificationPayload)).rejects.toThrow(
        'Invalid FCM push token: token is empty'
      );
    });
    ...
    it('should throw error on invalid token format', async () => {
      ...
      await expect(fcmService.sendMessage(invalidToken, mockNotificationPayload)).rejects.toThrow(
  ...
  describe('sendMessage - simulated FCM errors', () => {
    ...
    it('should throw token invalid error for test_fail_invalid token', async () => {
      const invalidToken =
        'test_fail_invalid_zcOQBtj:APA91bFo1X-8CcqQz8_qZ9xE_5K4xR2yL8mN9oP0qR1sT2uV3wX4yZ5aB6cD7eF8';
    ...
    it('should throw token expired error for test_fail_expired token', async () => {
      const expiredToken =
        'test_fail_expired_zcOQBtj:APA91bFo1X-8CcqQz8_qZ9xE_5K4xR2yL8mN9oP0qR1sT2uV3wX4yZ5aB6cD7eF8';

      await expect(fcmService.sendMessage(expiredToken, mockNotificationPayload)).rejects.toThrow(
        'Token expired in FCM'
      );
    });

    it('should throw rate limit error for test_fail_rate token', async () => {
      const rateLimitedToken =
        'test_fail_rate_zcOQBtj:APA91bFo1X-8CcqQz8_qZ9xE_5K4xR2yL8mN9oP0qR1sT2uV3wX4yZ5aB6cD7eF8';

      await expect(fcmService.sendMessage(rateLimitedToken, mockNotificationPayload)).rejects.toThrow(
        'FCM rate limit'
      );
    });
    ...
    it('should throw service unavailable error for test_fail_unavailable token', async () => {
      const unavailableToken =
        'test_fail_unavailable_zcOQBtj:APA91bFo1X-8CcqQz8_qZ9xE_5K4xR2yL8mN9oP0qR1sT2uV3wX4yZ5aB6cD7eF8';

      await expect(fcmService.sendMessage(unavailableToken, mockNotificationPayload)).rejects.toThrow(
        'FCM service unavailable'
      );
    });

    it('should throw authentication error for test_fail_auth token', async () => {
      const authErrorToken =
        'test_fail_auth_zcOQBtj:APA91bFo1X-8CcqQz8_qZ9xE_5K4xR2yL8mN9oP0qR1sT2uV3wX4yZ5aB6cD7eF8';

      await expect(fcmService.sendMessage(authErrorToken, mockNotificationPayload)).rejects.toThrow(
  ...
  describe('token validation', () => {
    ...
    it('should reject tokens with invalid characters', async () => {
      ...
      for (const token of invalidTokens) {
        await expect(fcmService.sendMessage(token, mockNotificationPayload)).rejects.toThrow(
          'Invalid FCM token format'
        );
      }
    });
