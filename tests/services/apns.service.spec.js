...
describe('APNsService', () => {
    ...
    describe('constructor and initialization', () => {
        it('should initialize with valid bundle ID', () => {
            ...
            apnsService = new APNsService_1.APNsService();
        ...
        it('should throw error when bundle ID is missing', () => {
            ...
            expect(() => {
                const service = new APNsService_1.APNsService(undefined, undefined, undefined, undefined, undefined);
                expect(service.isReady()).toBe(true);
            }).not.toThrow();
        });
        it('should throw error on invalid bundle ID format', () => {
            expect(() => {
                new APNsService_1.APNsService('', '', '', '', 'invalid_bundle_id_no_dots');
    ...
    describe('sendNotification - successful delivery', () => {
        ...
        it('should deliver payload with correct structure', async () => {
            ...
            expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('[APNs] Notification delivered'));
    ...
    describe('sendNotification - invalid token handling', () => {
        ...
        it('should throw error on empty token', async () => {
            await expect(apnsService.sendNotification('', mockNotificationPayload)).rejects.toThrow('Invalid APNs push token: token is empty');
        });
        it('should throw error on token with incorrect length', async () => {
            ...
            await expect(apnsService.sendNotification(tooShortToken, mockNotificationPayload)).rejects.toThrow('Invalid APNs token format');
        });
        it('should throw error on token with non-hex characters', async () => {
            ...
            await expect(apnsService.sendNotification(invalidToken, mockNotificationPayload)).rejects.toThrow('Invalid APNs token format');
        });
        it('should throw error on token with incorrect length but valid hex', async () => {
            ...
            await expect(apnsService.sendNotification(tooLongToken, mockNotificationPayload)).rejects.toThrow('Invalid APNs token format');
        });
    });
    describe('sendNotification - simulated APNs errors', () => {
        ...
        it('should throw token invalid error for test_fail_invalid token', async () => {
            ...
            await expect(apnsService.sendNotification(invalidToken, mockNotificationPayload)).rejects.toThrow('Invalid APNs token');
        });
        it('should throw token expired error for test_fail_expired token', async () => {
            ...
            await expect(apnsService.sendNotification(expiredToken, mockNotificationPayload)).rejects.toThrow('Token expired in APNs');
        });
        it('should throw rate limit error for test_fail_rate token', async () => {
            ...
            await expect(apnsService.sendNotification(rateLimitedToken, mockNotificationPayload)).rejects.toThrow('APNs rate limit');
        });
        it('should throw service unavailable error for test_fail_unavailable token', async () => {
            const unavailableToken = 'test_failunavail0123456789abcdef0123456789abcdef0123456789abcdef';
            await expect(apnsService.sendNotification(unavailableToken, mockNotificationPayload)).rejects.toThrow('APNs service unavailable');
        });
        it('should throw authentication error for test_fail_auth token', async () => {
            ...
            await expect(apnsService.sendNotification(authErrorToken, mockNotificationPayload)).rejects.toThrow('APNs authentication failed');
        });
    });
    describe('token validation', () => {
        ...
        it('should reject non-64-character tokens', async () => {
            ...
            for (const token of invalidTokens) {
                await expect(apnsService.sendNotification(token, mockNotificationPayload)).rejects.toThrow();
            }
        });
        it('should reject tokens with non-hex characters', async () => {
            ...
            for (const token of invalidTokens) {
                await expect(apnsService.sendNotification(token, mockNotificationPayload)).rejects.toThrow('Invalid APNs token format');
            }
        });
    });
