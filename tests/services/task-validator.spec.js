...
describe('TaskValidatorService', () => {
    ...
    beforeEach(() => {
        ...
        service = new TaskValidatorService_1.TaskValidatorService(mockTaskRepository, mockUserRepository);
    });
    describe('validateTaskCanExecute', () => {
        it('should validate user can execute task', async () => {
            const result = await service.validateTaskCanExecute('user_001', 'task_001', 'household_001');
        ...
        it('should reject if user not found', async () => {
            ...
            const result = await service.validateTaskCanExecute('user_999', 'task_001', 'household_001');
        ...
        it('should reject if task not found', async () => {
            ...
            const result = await service.validateTaskCanExecute('user_001', 'task_999', 'household_001');
            expect(result.isValid).toBe(false);
    ...
    describe('getAuthorizedHouseholdMembers', () => {
        it('should return all household members for valid task', async () => {
            const result = await service.getAuthorizedHouseholdMembers('task_001', 'household_001');
            expect(result).toHaveLength(2);
        });
        it('should exclude specified user', async () => {
            const ownerUserWithDifferentId = {
                ...ownerUser,
                userId: 'user_002',
            };
            mockUserRepository.findUsersInHousehold.mockResolvedValueOnce([
            ...
            const result = await service.getAuthorizedHouseholdMembers('task_001', 'household_001', 'user_001');
            expect(result).toHaveLength(1);
            expect(result[0].userId).toBe('user_002');
        });
        it('should return empty array if task not found', async () => {
            mockTaskRepository.findTaskById.mockResolvedValueOnce(null);
            const result = await service.getAuthorizedHouseholdMembers('task_999', 'household_001');
            expect(result).toHaveLength(0);
        });
        it('should return empty array on error', async () => {
            mockTaskRepository.findTaskById.mockRejectedValueOnce(new Error('DB error'));
    ...
    describe('validateRequiredFields', () => {
        ...
        it('should reject non-string userId', () => {
            expect(service.validateRequiredFields(123, 'task_001', 'household_001')).toBe(false);
        });
        ...
        it('should reject null values', () => {
            expect(service.validateRequiredFields(null, 'task_001', 'household_001')).toBe(false);
            expect(service.validateRequiredFields('user_001', null, 'household_001')).toBe(false);
            expect(service.validateRequiredFields('user_001', 'task_001', null)).toBe(false);
        });
        it('should reject undefined values', () => {
            expect(service.validateRequiredFields(undefined, 'task_001', 'household_001')).toBe(false);
            expect(service.validateRequiredFields('user_001', undefined, 'household_001')).toBe(false);
            expect(service.validateRequiredFields('user_001', 'task_001', undefined)).toBe(false);
        });
    });
