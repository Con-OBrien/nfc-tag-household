...
describe('EventRepository', () => {
    ...
    describe('logEvent', () => {
        ...
        it('should validate that userId is required', async () => {
            ...
            await expect(eventRepository.logEvent(event)).rejects.toThrow(types_1.ValidationError);
        });
    ...
    describe('getEventHistory', () => {
        beforeEach(async () => {
            ...
            const events = [
                { action: 'execute', timestamp: now - 3000 },
                { action: 'acknowledge', timestamp: now - 2000 },
                { action: 'undo', timestamp: now - 1000 },
                { action: 'comment', timestamp: now },
            ];
        ...
        it('should enforce household isolation', async () => {
            const otherHouseholdId = (0, crypto_1.randomUUID)();
        ...
        it('should require taskId', async () => {
            await expect(eventRepository.getEventHistory('', householdId)).rejects.toThrow(types_1.ValidationError);
        });
        it('should require householdId', async () => {
            await expect(eventRepository.getEventHistory(taskId, '')).rejects.toThrow(types_1.ValidationError);
        });
        it('should validate limit is within range', async () => {
            await expect(eventRepository.getEventHistory(taskId, householdId, 0)).rejects.toThrow(types_1.ValidationError);
            await expect(eventRepository.getEventHistory(taskId, householdId, 1001)).rejects.toThrow(types_1.ValidationError);
        });
    });
    describe('getEventsByUser', () => {
        beforeEach(async () => {
            // Create events from different users
            const users = [userId, (0, crypto_1.randomUUID)(), (0, crypto_1.randomUUID)()];
        ...
        it('should require userId', async () => {
            await expect(eventRepository.getEventsByUser('', householdId)).rejects.toThrow(types_1.ValidationError);
        });
        it('should require householdId', async () => {
            await expect(eventRepository.getEventsByUser(userId, '')).rejects.toThrow(types_1.ValidationError);
        });
    });
    describe('queryEventsByDateRange', () => {
        ...
        it('should enforce household isolation', async () => {
            const otherHouseholdId = (0, crypto_1.randomUUID)();
            const events = await eventRepository.queryEventsByDateRange(now - 5000, now, otherHouseholdId);
            expect(events.length).toBe(0);
        });
        it('should require householdId', async () => {
            await expect(eventRepository.queryEventsByDateRange(now - 5000, now, '')).rejects.toThrow(types_1.ValidationError);
        });
        it('should reject invalid date ranges', async () => {
            await expect(eventRepository.queryEventsByDateRange(now, now - 5000, householdId)).rejects.toThrow(types_1.ValidationError);
        });
        it('should limit date range to 90 days', async () => {
            const startTime = now - 100 * 24 * 60 * 60 * 1000; // 100 days ago
            const endTime = now;
            await expect(eventRepository.queryEventsByDateRange(startTime, endTime, householdId)).rejects.toThrow(types_1.ValidationError);
        });
        it('should return results in reverse chronological order', async () => {
            const events = await eventRepository.queryEventsByDateRange(now - 5000, now, householdId);
            for (let i = 0; i < events.length - 1; i++) {
                expect(events[i].timestamp).toBeGreaterThanOrEqual(events[i + 1].timestamp);
            }
        });
    ...
    describe('getEventsByAction', () => {
        ...
        it('should reject invalid action types', async () => {
            await expect(eventRepository.getEventsByAction(householdId, 'invalid')).rejects.toThrow(types_1.ValidationError);
        });
    });
    ...
    describe('countEvents', () => {
        ...
        it('should require householdId', async () => {
            await expect(eventRepository.countEvents('')).rejects.toThrow(types_1.ValidationError);
        });
    });
    ...
    describe('Household Isolation', () => {
        beforeEach(async () => {
            ...
            const households = [householdId, (0, crypto_1.randomUUID)(), (0, crypto_1.randomUUID)()];
        ...
        it('should isolate date range queries between households', async () => {
            const events = await eventRepository.queryEventsByDateRange(now - 1000, now + 1000, householdId);
