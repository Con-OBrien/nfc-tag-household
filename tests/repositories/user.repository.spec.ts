...
import { HouseholdUser, ValidationError } from '../../src/types';
...
describe('UserRepository', () => {
  ...
  describe('findUserById', () => {
    ...
    it('should require userId and householdId', async () => {
      await expect(userRepository.findUserById('', householdId)).rejects.toThrow(
        ValidationError
      );

      await expect(userRepository.findUserById(userId, '')).rejects.toThrow(ValidationError);
    });
  });
  ...
  describe('findUsersInHousehold', () => {
    ...
    it('should reject invalid role', async () => {
      await expect(
        userRepository.findUsersInHousehold(householdId, 'invalid' as any)
      ).rejects.toThrow(ValidationError);
    });
  ...
  describe('updateUserPushToken', () => {
    ...
    it('should require userId, householdId, and newToken', async () => {
      await expect(userRepository.updateUserPushToken('', householdId, 'token')).rejects.toThrow(
        ValidationError
      );

      await expect(userRepository.updateUserPushToken(testUserId, '', 'token')).rejects.toThrow(
        ValidationError
      );

      await expect(
        userRepository.updateUserPushToken(testUserId, householdId, '')
      ).rejects.toThrow(ValidationError);
    });
  });
  ...
  describe('updateUserPreferences', () => {
    ...
    it('should validate quiet hours format', async () => {
      await expect(
        userRepository.updateUserPreferences(testUserId, householdId, {
          quietHours: { start: 25, end: 26 } as any, // Invalid hours
        })
      ).rejects.toThrow(ValidationError);
    });
    ...
    it('should require userId and householdId', async () => {
      await expect(
        userRepository.updateUserPreferences('', householdId, {
          notificationsEnabled: false,
        })
      ).rejects.toThrow(ValidationError);

      await expect(
        userRepository.updateUserPreferences(testUserId, '', {
          notificationsEnabled: false,
        })
      ).rejects.toThrow(ValidationError);
    });
