...
describe('EventController', () => {
    ...
    beforeEach(() => {
        ...
        eventController.eventRepository = mockEventRepository;
        eventController.userRepository = mockUserRepository;
    });
    describe('getHouseholdEvents', () => {
        it('should return household events with pagination', async () => {
            ...
            mockEventRepository.getHouseholdEvents.mockResolvedValue(mockEvents);
            const req = {
        ...
        it('should enforce household isolation', async () => {
            mockUserRepository.isUserInHousehold.mockResolvedValue(false);
            const req = {
                params: { householdId: testIds.householdId },
                query: {},
                userId: testIds.userId1,
        ...
        it('should filter events by eventType', async () => {
            ...
            mockEventRepository.getHouseholdEvents.mockResolvedValue([
                executeEvent,
                acknowledgeEvent,
            ]);
            const req = {
        ...
        it('should handle date range filtering', async () => {
            ...
            mockEventRepository.queryEventsByDateRange.mockResolvedValue(mockEvents);
            const req = {
                params: { householdId: testIds.householdId },
                query: {
            ...
            const next = jest.fn();
            await eventController.getHouseholdEvents(req, res, next);
            expect(mockEventRepository.queryEventsByDateRange).toHaveBeenCalledWith(now - 20000, now, testIds.householdId);
            expect(res.status).toHaveBeenCalledWith(200);
        });
    ...
    describe('getUserEvents', () => {
        it('should return user events across households', async () => {
            ...
            mockUserRepository.findUserDetails.mockResolvedValue({
                userId: testIds.userId1,
                name: 'Test User',
            });
            mockUserRepository.getUserHouseholds.mockResolvedValue([
                { householdId: testIds.householdId },
            ]);
            mockEventRepository.getEventsByUser.mockResolvedValue([
                mockEvent1,
                mockEvent2,
            ]);
            const req = {
        ...
        it('should filter user events by type', async () => {
            ...
            await eventController.getUserEvents(req, res, next);
            const responseData = res.json.mock.calls[0][0].data;
            const filteredEvents = responseData.events.filter((e) => e.action === 'execute');
            expect(filteredEvents.length).toBeGreaterThan(0);
        });
        it('should return user not found error if user does not exist', async () => {
            mockUserRepository.findUserDetails.mockResolvedValue(null);
            const req = {
                query: { skip: '0', limit: '50' },
